...
def fecharpedido():
    ...
    if conta >= 10:
        ...
        entrega = Label(text='Seu pedido será entregue no endereço cadastrado\n dentro de 1 hora.',font=('Arial',13,'bold'),bg='gold')
        entrega.place(x=1050,y=727)
    concluir ['state'] = DISABLED

    if conta == 0:
        totaldaconta = Label(text='Escolha ao menos um produto para poder concluir o pedido.',font=('Arial',11,'bold'),bg='red')
        totaldaconta.place(x=1100,y=527)

    if conta > 0 and conta < 10:
        totaldaconta = Label(text='O pedido mínimo é de R$10,00. Selecione mais produtos.',font=('Arial',12,'bold'),bg='red')
        totaldaconta.place(x=1100, y=527)
